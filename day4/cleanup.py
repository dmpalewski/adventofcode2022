...
def is_left_within_right(left: Range, right: Range) -> bool:
    return right.lower_bound <= left.lower_bound and right.upper_bound >= left.upper_bound
...
def process_input(path: Path) -> int:
    ...
    with open(path, "r") as fin:
        for line in fin.readlines():
            ...
            is_fully_contained = is_left_within_right(left, right) or is_left_within_right(right, left)
